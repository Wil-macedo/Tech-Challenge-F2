...
tempPath = os.path.join(os.path.abspath(os.curdir), "temp_files") + (
    os.path.sep
)  # Temp dir
...
def getcsv():
    ...
    for i in range(0, offset):  # Pega próximos dias do offset.
        try:
            if dateParam is None or not re.match(
                r"^(\d{4})-(\d{2})-(\d{2})$", dateParam
            ):
                # TODO: Se não tiver data, podemos pegar o ultimo dia útil talvez :/
                dateParam = "2024-08-31"

            base_url = "https://arquivos.b3.com.br/tabelas"
            urlGet = f"{base_url}/TradeInformationConsolidatedAfterHours/{dateParam}"
            driver.get(urlGet)
            ...
            download_button = WebDriverWait(driver, 20).until(
                EC.element_to_be_clickable(
                    (
                        By.XPATH,
                        "/html/body/div/div/div/div/div[2]/div[3]/div[1]/div[2]/p/a",
                    )
                )
            )
            ...
            while temp_files:
                time.sleep(0.5)
                temp_files = [
                    f for f in os.listdir(tempPath) if f.endswith(".crdownload")
                ]
            ...
            for file in os.listdir(tempPath):
                ...
                if match:
                    date = match.group(1)
                    fileName = (
                        date[:4] + "-" + date[4:6] + "-" + date[6:8] + ".parquet"
                    )  # 2024-08-30
                    ...
                    result += (
                        f"OK - DATA: {dateParam} - Arquivo baixado com sucesso, "
                        f"convertido em parquet & movido - {fullPath} <br><br>"
                    )

                else:
                    os.remove(source_file)
                    result += (
                        f"{dateParam} - Não há informações para essa data, "
                        f"**DELETED** <br><br>"
                    )
